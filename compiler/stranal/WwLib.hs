...
mkWWcpr_one_layer fam_envs body_ty term cpr = runMaybeT $ do
  (con_tag, arg_terms, arg_cprs) <- MaybeT $ return $ returnsCPR_maybe term cpr

  (data_con, inst_tys, arg_tys, co) <- case deepSplitCprType_maybe fam_envs con_tag body_ty of
    Just stuff -> return stuff
    Nothing    -> WARN( True, text "mkWWcpr: non-algebraic or open body type" <+> ppr body_ty )
                  mzero
...
mk_absent_let dflags arg
  ...
  where
    lifted_arg   = arg `setIdStrictness` botSig `setIdCprInfo` botCpr
